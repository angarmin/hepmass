...
import os 
#cwd = os.getcwd()
cwd="/home/angela/Notebook/machine_learning/normalizados/RF_unbalanced/"
...
#Metric df: 
df_metrics = pd.DataFrame(index=[500,750,1000,1250,1500], columns=['accuracy','sensitibity','specifity','kappa','auc'])#With other masses
...
for i in (500,750,1000,1250,1500):
    ...
    ##THE PROPORTION WILL BE 1000(BG) TO 1 (SIGNAL)
    dfmass1000test=df_originaltest.loc[df_originaltest['mass'] == i].sample(n=dfmass0test.shape[0]//1000, random_state=1)
    dfmass1000test=pd.concat([dfmass1000test, dfmass0test]).sample(frac=1).reset_index(drop=True)
    
    #remember, train proportion: 70-35 = 67-33, test is the half part of train.     
    dfmass1000test=dfmass1000test.drop('mass', axis=1).sample(n=dfmass1000.shape[0]//2, random_state=1)
